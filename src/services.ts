...
export class TempFrameDrawingService extends BaseTempFrame implements IService{
    ...
    public OnDrawingFrameStarted=(e:ICustomMouseEvent)=>{
...
export class TempFrameService implements IService{
    ...
    public get TempFrame(){ return this._TempFrame?.component ?? null}
    public constructor(){
    ...
    public OnMoveBegin=(e: { detail: MouseEvent & DomEvent })=>{
        if(S.TEMPFRAME.HAS(S.TEMPFRAME.MOVING)){
           ...
           if(ST.ALL(ST.SHOW)&&!ST.SOME(ST.RESIZE_DOING,ST.RESIZE_BEGIN)) {
               ...
               if (edge) {
                   ...
                   this.resizeBegin={
                        ...
                        div:{left:this.TempFrame.getBoundingClientRect().left,
                            top:this.TempFrame.getBoundingClientRect().top,
    ...
    public OnResizing=(e: { detail: MouseEvent & DomEvent })=>{
        ...
        if(ST.ALL(ST.RESIZE_TRIGGERED,ST.RESIZE_DOING)){
            let [sx,sy,ex,ey] = [window.scrollX,window.scrollY,e.detail.clientX,e.detail.clientY]
