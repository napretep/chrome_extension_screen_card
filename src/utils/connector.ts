...
export  const SetupCoreEventEmitter=()=>{
    ...
    state.registEvents={
        EVENT_FRAME_DRAWING_TRIGGERED:()=>{S.FRAME_DRAWING.ADD(S.FRAME_DRAWING.TRIGGERED)},
        ...
        EVENT_FRAME_SHOW:()=>{S.TEMPFRAME.SetBasicState()},
        EVENT_FRAME_HIDE:()=>{S.TEMPFRAME.CLEAR()},

        EVENT_FRAME_MOVE_BEGIN:()=>{S.TEMPFRAME.ADD(S.TEMPFRAME.MOVE_BEGIN)},
        EVENT_FRAME_MOVING:()=>{S.TEMPFRAME.ADD(S.TEMPFRAME.MOVE_DOING)},
        EVENT_FRAME_MOVE_END:()=>{S.TEMPFRAME.RemoveGroup("MOVE")},

        EVENT_FRAME_RESIZE_TRIGGERED:()=>{S.TEMPFRAME.ADD(S.TEMPFRAME.RESIZE_TRIGGERED)},
        ...
        EVENT_FRAME_RESIZE_END:()=>{S.TEMPFRAME.RemoveGroup("RESIZE")},

        EVENT_FRAME_AT_BUTTON:()=>{S.TEMPFRAME.ADD(S.TEMPFRAME.AT_BUTTON)},
        EVENT_FRAME_OUT_BUTTON:()=>{S.TEMPFRAME.DEL(S.TEMPFRAME.AT_BUTTON)},

        EVENT_MOUSE_LBTN_PRESSED:()=>{S.MOUSE.ADD(S.MOUSE.LBTN_HOLDING)},
        EVENT_MOUSE_LBTN_RELEASED:()=>{S.MOUSE.DEL(S.MOUSE.LBTN_HOLDING)},
        EVENT_FRAME_TOGGLE_FOLDBODY:()=>{
            if(!S.TEMPFRAME.HAS(S.TEMPFRAME.BTN_FOLDBODY))
                S.TEMPFRAME.ADD(S.TEMPFRAME.BTN_FOLDBODY)
        else S.TEMPFRAME.DEL(S.TEMPFRAME.BTN_FOLDBODY)},
        EVENT_FRAME_TOGGLE_MINIMIZE:()=>{
            if(!S.TEMPFRAME.HAS(S.TEMPFRAME.BTN_MINIMIZE))
                S.TEMPFRAME.ADD(S.TEMPFRAME.BTN_MINIMIZE)
        else S.TEMPFRAME.DEL(S.TEMPFRAME.BTN_MINIMIZE)},
        EVENT_FRAME_TOGGLE_TOOLS:()=>{
            if(!S.TEMPFRAME.HAS(S.TEMPFRAME.BTN_TOOLS))
                S.TEMPFRAME.ADD(S.TEMPFRAME.BTN_TOOLS)
        else S.TEMPFRAME.DEL(S.TEMPFRAME.BTN_TOOLS)},
        EVENT_FRAME_TOGGLE_PINE:()=>{
            if(!S.TEMPFRAME.HAS(S.TEMPFRAME.BTN_PINE))
                S.TEMPFRAME.ADD(S.TEMPFRAME.BTN_PINE)
        else S.TEMPFRAME.DEL(S.TEMPFRAME.BTN_PINE)},
